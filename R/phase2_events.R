...
panel_node_events <- function(data = NULL,
  ...
  program_events %>% group_by(.data$Container) %>%
  ...
  # Just to Make sure
  matched_fxt_flush_events %>% filter(.data$Value==.data$Last_Value) %>% nrow() -> is_wrong
  if(is_wrong > 0){
    starvz_warn("Something wrong matching fxt flush events")
  }

  matched_fxt_flush_events %>% select(.$Container) %>%
                               distinct() %>%
  ...
  matched_fxt_flush_events %>%
    mutate(y=as.numeric(.data$Container)+0.1,
           ymax=.data$y+0.9) %>%
           ggplot(aes(ymin=.data$y, ymax=.data$ymax,
