...
time_aggregation_prep <- function(dfw = NULL)
{
    ...
    dfw_zero_1 <- dfw_initial %>% slice(1) %>% mutate(StartN = 0, EndN = .data$Start, Value = 0);
...
time_aggregation_post <- function(dfw = NULL, dfw_agg = NULL)
{
    ...
    df_ResourceId <- dfw %>%
        select(.data$ResourceId, .data$Type,
...
node_spatial_aggregation <- function(dfw) {
  if (is.null(dfw)) return(NULL);
  dfw %>% group_by(.data$Node, .data$ResourceType) %>%
          mutate(N = n_distinct(.data$ResourceId)) %>%
          ungroup() %>%
      group_by(.data$Node, .data$Slice, .data$Task, .data$ResourceType,
...
st_time_aggregation <- function(dfw = NULL, StarPU.View = FALSE, step = 100)
{
    ...
    dfwagg_full %>%
        ...
        mutate(TaskHeight = (.data$Height*0.9 * .data$Value),
...
aggregate_trace <- function(df_native, states, excludeIds, min_time_pure) {
    df_native <- df_native %>% group_by(.data$ResourceId) %>%
    ...
    df_native2 <- df_native %>% group_by(.data$ResourceId, .data$Chunk) %>% mutate(Start=head(.data$Start,1), End=tail(.data$End, 1))
    df_aggregate <- df_native2 %>% group_by(.data$ResourceId, .data$Chunk, .data$Value) %>%
    ...
    df_aggregate <- df_aggregate %>% group_by(.data$ResourceId, .data$Chunk) %>% mutate(Activity = .data$Duration/(.data$End-.data$Start))
    df_aggregate <- df_aggregate %>% group_by(.data$ResourceId, .data$Chunk) %>% mutate(aggregated=ifelse((abs(.data$Activity-1) <= 0.00001) & .data$Number==1, FALSE, TRUE))
    df_aggregate
}
...
compute_aggregate_coord <- function(df){
    ...
    df %>% group_by(.data$ResourceId, .data$Chunk) %>% mutate(PosY=(as.numeric(.data$ResourceId)-0.4)+(cumsum(.data$Activity))) # -0.4 is needed to put the rectangle in the middle of the y-axi
}
geom_aggregated_states <- function (data = NULL, Show.Outliers = FALSE, min_time_pure = 1000, states = NA, base_size=22, labels = "1", expand_value=0.05)
{
    ...
    ret[[length(ret)+1]] <- scale_y_continuous(breaks = yconfm$Position+(yconfm$Height/3), labels=yconfm$ResourceId,
                                               expand=c(expand_value,0));

    ret[[length(ret)+1]] <- geom_rect(data=dfw, aes(fill=.data$Value,
                                                    xmin=.data$Start,
                                                    xmax=.data$End,
                                                    ymin=.data$Position,
...
st_time_aggregation_vinicius_plot <- function (data = NULL)
{
    ...
    # The longer outlier
    longer.outlier = data$Application %>% filter(.data$Outlier == TRUE) %>% pull(.data$Duration) %>% max;
...
st_time_aggregation_plot <- function (data = NULL, dfw_agg = NULL, StarPU.View = FALSE)
{
    ...
    gow <- dfw_agg %>% ggplot() +
        ...
        geom_rect(aes(fill=.data$Task,
                      xmin=.data$Start,
                      xmax=.data$End,
                      ymin=.data$Position+.data$TaskPosition,
                      ymax=.data$Position+(.data$TaskPosition+.data$TaskHeight), color=.data$Task ), alpha=.5) +
        # Print outliers on top
        geom_rect(data=(dfw %>% filter(.data$Outlier == TRUE)),
                  aes(fill=.data$Value,
                      xmin=.data$Start,
                      xmax=.data$End,
                      ymin=.data$Position,
                      ymax=.data$Position+.data$Height-0.4), alpha=1);
