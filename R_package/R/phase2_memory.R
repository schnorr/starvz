...
events_memory_chart <- function (data = NULL, globalEndTime = NULL, combined = FALSE, tstart = NULL, tend = NULL)
{
    ...
    dfwapp = data$Events_memory %>%
          ...
          group_by(.data$Container, .data$Handle, .data$Tid, .data$Src) %>% arrange(.data$Start) %>%
                        ...
                        tend=NULL)
{
    ...
    col_pos_1 <- data.frame(Container=unique(dfl$Dest)) %>% arrange(.data$Container) %>% rowid_to_column("Position");

    col_pos_2 <- data.frame(Container=unique(dfw$Container)) %>% arrange(.data$Container) %>% rowid_to_column("Position")
    ...
    yconfm <- yconfm %>% unnest() %>% arrange(.data$Container)
	...
	dx <- dfw %>% filter(.data$Type == "Allocating") %>%
	...
	dx$Coordinates <- gsub(" ", "x", dx$Coordinates)
      ...
      ret[[length(ret)+1]] <- geom_text(data=dx, colour = "black", fontface = "bold",
       aes(x = .data$Start+.data$Duration/2, y = .data$Position+(2.0-0.2-.data$Height)/2,
                  label=.data$Coordinates), size = 5, alpha=1.0,
    ...
    if(main_data$config$memory$state$total){
      ...
      ms <- dfw %>% filter(.data$Type == select, .data$Start<tend, .data$End>tstart) %>%
      ...
      if(nrow(ms) != 0){
          ...
          ret[[length(ret)+1]] <- geom_label(data=ms, x = globalEndTime, colour = "black", fontface = "bold",
                        ...
                        total_active = FALSE)
{
    ...
    if(border_active){
        ret[[length(ret)+1]] <- geom_segment(data=dfl,
           aes(x = .data$Start, xend = .data$End, y = .data$O_Position, yend = .data$D_Position), arrow = arrow_g, alpha=0.5, size=1.5, color = "black");
    }

    ret[[length(ret)+1]] <- geom_segment(data=dfl, aes(x = .data$Start, xend = .data$End,
        y = .data$O_Position, yend = .data$D_Position, color = .data$Origin), arrow = arrow_g, alpha=1.0);
    selected_dfl <- dfl %>% filter(.data$End > tstart) %>%  filter(.data$Start < tend);
    ...
    if(total_active){
      ...
      if(nrow(total_links) != 0 & !combined){
          ...
          ret[[length(ret)+1]] <- geom_label(data=total_links, x = globalEndTime, colour = "white", fontface = "bold",
...
handles_presence_states <- function(data){
    ...
    data$Events_data %>% filter(.data$Type=="data state invalid" |
    ...
    data_state_events %>% group_by(.data$Value, .data$Container) %>%
        mutate(rep = case_when(.data$Type=="data state owner" ~ 1,
...
# that will not match between MPI executions...
    ...
    data$Events_memory <- data$Events_memory %>%
        ...
        mutate(Type=case_when(.data$Type=="Allocating Start" ~ "Allocation Request",
    ...
    p_data %>% select(.data$Container, .data$Value, .data$y1) %>% distinct() -> pre_p_data

    data$Task_handles %>% filter(!is.na(.data$JobId)) %>%
    ...
    if(TR %>% nrow() > 0){
        ...
        TR %>%
            ...
            mutate(Container=paste0(.data$G, .data$P)) %>% select(-.data$P, -.data$G) %>%
            select(-.data$Tid) %>%
    ...
    data$Task_handles %>% select(.data$Handles) %>% distinct() %>% .$Handles -> h_used
    data$Events_memory %>%  filter(.data$Handle %in% h_used) %>% select(-.data$Tid) %>%
        inner_join(data$Data_handle, by=c("Handle" = "Handle") ) %>%
        inner_join(position, by=c("Container"="Container")) %>%
        name_func() %>%
        select(.data$Container, .data$Type, .data$Start, .data$Value, .data$Info, .data$y1) %>%
    ...
    allocation_events  %>% group_by(.data$Value, .data$Container, .data$Type) %>%
        mutate(Old = lag(.data$Start, default=-5), R=abs(.data$Start-.data$Old)) %>%
        filter(.data$R>1) %>% select(-.data$Old,-.data$R)  -> allocation_events_filtered
    ...
    data$Events_memory %>% filter(.data$Type=="DriverCopy Start") %>%
        select(.data$Handle, .data$Info, .data$Container) %>%
        mutate(Info=as.integer(.data$Info)) -> links_handles

    mpi_links <- data$Link %>% filter(.data$Type=="MPI communication") %>%
        ...
        rename(Transfer = .data$Type) %>% unique()

    links <- data$Link %>% filter(.data$Type=="Intra-node data Fetch" |
...
handles_gantt <- function(data, JobId=NA, lines=NA, lHandle=NA){
    ...
    if(is.na(JobId) && is.na(lHandle)){
        ...
        myjobid = JobId
        data$Task_handles %>% filter(.data$JobId==myjobid) %>%
            inner_join(data$Data_handle, by=c("Handles" = "Handle") ) %>%
            ungroup() %>% .$name_func() %>% .$Value -> selected_handles
...
memory_snap <- function(data, selected_time, step, tasks_size=30){
	...
	data[[3]] %>% select(.data$Value, .data$Color) %>% distinct() -> c_info
	...
	data[[1]] %>% filter(.data$Start < selected_time, .data$End > selected_time) %>%
		      right_join(data[[2]], by=c("Value" = "Handle", "Container"="Container")) -> d_presence

	task_presence <- data[[3]] %>% filter(.data$Start <= selected_time, .data$End >= selected_time) %>%
		         inner_join(data[[2]], by=c("Handles" = "Handle", "Container"="Container"))

	task_presence_alpha <- data[[3]] %>% filter(.data$Start > selected_time-step, .data$End <= selected_time) %>%
		         inner_join(data[[2]], by=c("Handles" = "Handle", "Container"="Container"))
