...
{
    node <- dfl %>% slice(1) %>% pull(.data$Node);
    ...
    lpresult %>%
        ...
        mutate(Count = result[[1]]$solution[1:nrow(.data)],
               Estimation = TRUE) %>%
        rename(ResourceType = .data$Types,
               Value = .data$Values) -> ret;
...
abe_cpu_cuda_inner <- function(dfl, debug=FALSE)
{
  ...
  # The amount of nodes
  nnodes = dfl %>% select(.data$Node) %>% unique %>% .$Node %>% sort %>% length;
  ...
  df1.num_mean <- dfl %>% group_by(.data$ResourceType, .data$Value) %>%
    summarize(Num = n(), Mean=mean(.data$Duration)) %>% as.data.frame();
  ...
  values <- df1.num_mean %>% select(.data$Value) %>%
                             arrange(.data$Value) %>% .$Value %>% unique;
  types <- df1.num_mean %>% select(.data$ResourceType) %>%
                            arrange(.data$ResourceType) %>% .$ResourceType %>% unique %>% as.character;
  ...
  m.dir1 <- rep("=", nvalues);
  m.rhs1 <- df1.num_mean %>% group_by(.data$Value) %>% summarize(Sum = sum(.data$Num)) %>% arrange(.data$Value) %>% .$Sum;
  ...
  m <- df1.num_mean %>%
      ...
      pivot_wider(id_cols=.data$ResourceType, names_from = .data$Value, values_from = .data$Mean,
...
hl_global_cpb <- function (data = NULL)
{
    ...
    dfdag %>%
        ...
        # Rename things
        rename(JobIdStr = .data$JobId, DepStr = .data$Dependent,
               JobId = .data$JobIdIntU, Dependent = .data$DepIntU) %>%
        as_tibble() -> appdagcost;

    # Define the origin (the first task in the trace)
    stask <- appdagcost %>% arrange(.data$Start) %>% slice(1) %>% as.data.frame() %>% .$JobId;
    tasksOnCriticalPath <- sort(boost_shortest_path (stask, appdagcost));
    tasksOnCriticalPath <- identifiers %>% filter(.data$JobIdInt %in% tasksOnCriticalPath) %>% .$JobId %>% unique

    # Gather the CPB, sum the durations, return
    States <- bind_rows(data$Application, data$Starpu)
    States %>%
        filter(.data$JobId %in% tasksOnCriticalPath) %>% pull(.data$Duration) -> sel1;
    sel1 %>% sum -> sum1;
    if(!is.null(data$Link)){
        data$Link %>%
            filter(.data$Key %in% tasksOnCriticalPath) %>% pull(.data$Duration) -> sel2;
        sel2 %>% sum -> sum2;
        ret <- list(
            "CPBMPI" = sum1+sum2,
            "NMPI" = sel2 %>% length);
...
calculate_resource_idleness <- function(dfw = NULL, max_only = TRUE)
{
    ...
    dfw <- dfw %>%
            distinct(.data$ResourceType, .data$ResourceId, .data$Node,
                     ...
                     .data$Value, .data$Duration, .keep_all=TRUE);
    ...
    dfw <- dfw %>%
        ...
        summarize(Idleness = round((1-(sum(.data$End-.data$Start)/total_time))*100,2),
                  End = max(.data$End));
    if(max_only){
        dfw <- dfw %>% group_by(.data$Node, .data$ResourceType) %>%
...
geom_idleness <- function(data = NULL)
{
    ...
    globalEndTime <- dfidle %>% pull(.data$End) %>% na.omit %>% max;
    ret <- NULL;
    ret <- geom_label(data=dfidle,
                     ...
                     aes(y=.data$Position+(.data$Height/2.5), # vertical
...
geom_cpb_internal <- function(dfw = NULL, value = NULL, desc = NULL, bsize=22)
{
    ...
    if (!is.null(value) && !is.null(desc)){
        ...
        minPos = dfw %>% select(.data$Position) %>% na.omit %>% pull(.data$Position) %>% min;
        maxPos = dfw %>% select(.data$Position) %>% na.omit %>% pull(.data$Position) %>% max;
        corr = dfw %>% select(.data$Height) %>% na.omit %>% pull(.data$Height) %>% min;
        ret <- list(
            # the gray band
                              ...
                              label = TRUE)
{
  ...
  if (!is.null(pernodeABEdf)){
      ret <- list(
          geom_segment(data=pernodeABEdf, aes(x = .data$Result, xend=.data$Result,
                             y = .data$MinPosition, yend=.data$MaxPosition),
                             size=abesize, alpha=.7, color=bar_color)
      )
      if(text){
          ret <- list(ret,
                      geom_text (data=pernodeABEdf, aes(x = .data$Result,
                         y = .data$MinPosition+(.data$MaxPosition-.data$MinPosition)/2,
                      label=paste0(ifelse(label, "ABE: ", ""), round(.data$Result, 0))), angle=90, color="black", size=bsize)
                      )
      }
      return(ret);
  }
...
starpu_freq_abe_all_info <- function(tasks_per_node, resources_extra_info){
    ...
    resources_extra_info %>% select(.data$ResourceType, .data$codelet)  %>%
       unique() %>%
    ...
    codelets <- codelet_combination %>% select(.data$codelet) %>% unique() %>% .$codelet

    set_codelet <- function(codelet_name, codelet_combination){
        codelet_combination %>%
          mutate(v = case_when(.data$codelet == codelet_name ~ 1L,
                               TRUE ~ 0L)) %>% pull(.data$v) -> value
          # add zero for time
          return(c(value, 0))
    }

    types <- resources_extra_info %>% arrange(.data$codelet) %>%
          select(.data$ResourceType) %>% unique() %>% .$ResourceType

    set_resource <- function(resource_name, resources_extra_info){
         resources_extra_info  %>%
          mutate(v = case_when(.data$ResourceType == resource_name ~ (mean/n),
                               TRUE ~ 0)) %>% pull(.data$v) -> value
          # add -1 for time
          return(c(value, -1))
    }
    ...
    tasks_per_node %>% arrange(.data$Value) %>% .$freq -> tasks_freq
...
starpu_apply_abe_per_slice <- function(step, resources_extra_info, tasks, max_res=NULL){
    if(!is.null(max_res)){
        max_res %>% filter(.data$Step==step) %>% .$ResourceType -> rt
