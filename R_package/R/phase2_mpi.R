...
concurrent_mpi <- function(data = NULL)
{
    ...
    dflink %>%
        ...
        group_by(.data$Origin) %>% mutate(End = lead(.data$Start)) %>% na.omit %>% mutate(Duration = .data$End-.data$Start) %>% ungroup() %>%
...
concurrent_mpi_out <- function(data = NULL)
{
    ...
    dflink %>%
        ...
        select(-.data$Start) %>%
        rename(Start =.data$ Timestamp) %>%
        group_by(.data$Dest) %>% mutate(End = lead(.data$Start)) %>% na.omit %>% mutate(Duration = .data$End-.data$Start) %>% ungroup() %>%
        mutate(Type = "MPI Concurrent") %>%
