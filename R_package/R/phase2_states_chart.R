...
k_chart <- function (data = NULL, middle_lines = NULL, per_node = FALSE, colors = NULL)
{
    ...
    goijk <- dfw %>% ggplot() +
        ...
        geom_curve(data=dfborders, aes(x=.data$Start, xend=.data$StartB,
          y=.data$Iteration+height-height/2, yend=.data$IterationB+height-height/2), curvature=0.1, angle=20) +
        # The end border
        geom_curve(data=dfborders, aes(x=.data$End, xend=.data$EndB,
          y=.data$Iteration-height/2, yend=.data$IterationB-height/2), curvature=-0.1, angle=20) +
        # The state
        geom_rect(aes(fill=.data$Value,
                      xmin=.data$Start,
                      xmax=.data$End,
                      ymin=.data$Iteration-height/2,
                      ymax=.data$Iteration+height/2), alpha=.5)
...
geom_path_highlight <- function (paths = NULL)
{
    ...
    ret[[length(ret)+1]] <- geom_rect(data=paths,
                                      ...
                                      aes(color=.data$Path,
                                          ...
                                          xmax=.data$End,
                                          ymin=.data$Position,
                                          ymax=.data$Position+.data$Height-0.2), alpha=0);

    # let's draw lines connecting tasks in the path

    # collect each JobId coordinates
    paths %>% select(.data$JobId, .data$Start, .data$End, .data$Position, .data$Height) %>% unique -> x1;
...
node_summary <- function(data){
    ...
    data$Application %>% group_by(.data$Node) %>%
               arrange(-.data$End) %>% slice(1) %>%
               select(.data$Node, .data$End) %>% ungroup() %>%
    ...
    Abes %>% select(.data$Node, .data$Result) %>%
     ...
     Nodes <- all_data %>% mutate(Node = as.integer(.data$Node)) %>% .$Node %>% max()
     all_data %>% mutate(Node = as.integer(.data$Node)) %>%
...
hl_per_node_ABE(data$Application) %>%
    ...
    summarize(Node.Position = min(.data$ResourceType.Position),
